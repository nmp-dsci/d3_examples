...
# Mapping 
# map_df = pd.DataFrame(buffer_r[0]['geometry']['coordinates'][0])
# ax = map_df.plot(x=0,y=1)
# ax.scatter([map_df[0].mean()],[map_df[1].mean()])
# plt.show()

shapedf = reader.shapeRecords()

dir(shapedf[0].record)

poa_l = [x.record[0] for x in shapedf]
